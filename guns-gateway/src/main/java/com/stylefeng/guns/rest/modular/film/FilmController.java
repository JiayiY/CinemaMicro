...
public class FilmController {
     ...
     * @Author yangjiayi
     * @Description // 获取首页信息接口 功能聚合【API聚合】
     * 好处：1. 六个接口，一次请求；
     * 2. 同一个接口对外暴露，降低了前后端分离开发的难度和复杂度
     * 坏处：一次请求的数据过多 可能出问题
     * @Date 11:59 2020/7/19
     */
    ...
    public ResponseVO getConditionList(@RequestParam(name = "catId", required = false, defaultValue = "5") String catId,
                                       @RequestParam(name = "sourceId", required = false, defaultValue = "5") String sourceId,
                                       @RequestParam(name = "yearId", required = false, defaultValue = "5") String yearId) {
        ...
        for (SourceVO sourceVO : sourceVOList) {
            if ("5".equals(sourceVO.getSourceId())) {
                source = sourceVO;
                continue;
            }
    ...
    public ResponseVO<?> films(@PathVariable("searchParam") String searchParam, int searchType) throws InterruptedException, ExecutionException {
        ...
        // 根据searchType，判断查询类型 id:到redis中查 名称:到es中查
